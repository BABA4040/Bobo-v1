...
name: member.name})}
    ...
    if(author.money < Number(args[2])) return message.channel.send({content:`❎ You don't have this amount credit!`})
  ...
  member.send({content: `🏧 | Transfer Receipt \`You have received __\`$${args[2]}\`__ From user ${message.author.username} (ID: ${message.author.id})\``})
    message.channel.send({content:`**${message.author.username}** send credit to **${member.username}** amount: __\`$${args[2].toLocaleString()}\`__ ${m}`})
    }};
