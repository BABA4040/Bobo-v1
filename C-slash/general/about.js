...
prime: false,
  run: async (interaction,bot,data) => {
    ...
    const messages_cached = interaction.channels.cache
    .filter(x => x.send )
    .reduce((m, c) => m + c.messages.cache.size, 0);

    const canvas = createCanvas(400,250);
    const ctx = canvas.getContext('2d');
    const avatar = await loadImage(bot.user.displayAvatarURL({ format: 'png', size: 64 }));
    ...
    ctx.fillText('#' + bot.user.discriminator, 100, 45, 50);
    ...
    ctx.fillStyle = 'rgba(48, 4, 110, 0.9)';
    ctx.fillText(text.commatize(bot.messages.received), 200, 125, 100);
    ctx.fillText(text.commatize(bot.messages.sent), 200, 155, 100);
    ctx.fillText(text.commatize(bot.guilds.cache.size), 200, 185, 100);
    ctx.fillText(text.commatize(bot.commands.size), 200, 215, 100);
...
//===================================================
    ...
    ctx.beginPath();
    ctx.lineWidth = 15;
    ctx.arc(75,150,50, Math.PI * 1.5, Math.PI * 1.5 + (Math.PI * 2 * (messages_cached / (bot.channels.cache.size * bot.options.messageCacheMaxSize))));
    ...
    ctx.textAlign = 'center';
    ctx.fillText(Math.round(100 - ((messages_cached / (bot.channels.cache.size * bot.options.messageCacheMaxSize)) * 100)) , 75, 150, 40);
    ctx.font = '10px arial';
...
//===================================================
    ...
    ctx.fillText(`Uptime: ${moment.duration(bot.uptime, 'milliseconds').format('D [days] H [hours] m [minutes]')}`, 10, canvas.height - 5, 175);
    ctx.textAlign = 'right';
    ctx.fillText(`Created: ${moment(bot.user.createdAt).format('dddd, Do MMMM YYYY')}` ,canvas.width - 10, canvas.height - 5, 220)

    const attachment = canvas.toBuffer();
    const name = 'bot_stats.png';

    return interaction.reply('Bot Stats for Nerds', {
